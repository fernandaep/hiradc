            ...
            <div class="card">
              ...
              <template #cell(updated_at)="row">
                ...
                <template #cell(actions)="row">
                  ...
                  <b-button variant="outline-danger" size="sm" @click="deleteUnit(row.item.id)">
              ...
              <b-modal @shown="focusMyElement" ref="my-modal" :id="infoModal.id" :title="infoModal.title" @hide="resetInfoModal" hide-footer>
                <form @submit.prevent="editMode ? update() : store()"> 
                  <div class="modal-body">
                    <b-form-group id="example-input-group-1" label="Unit" label-for="unit_kerja">
                      <b-form-input
                        id="unit_kerja"
                        name="unit_kerja"
                        ref="unit_kerjaReff"
  ...
  export default {
    ...
    methods: {
     loadData() {
      ...
      validateState(unit) {
        const { $dirty, $error } = this.$v.form[unit];
      ...
      async store() {
         this.$v.form.$touch();
          ...
          try {
            let response =  await axios.post('api/unit',this.form)
             //console.log(response.status);
              if(response.status==200){
                  ...
                  this.$swal({
                    icon: 'success',
                    title: 'Unit Kerja Added successfully'
                  });
                  this.loadData();
              }
          } catch (e) {
      ...
      async update() {
        this.$v.form.$touch();
          ...
          try {
            let id = this.form.id;
            let updated = await axios.put('api/unit/'+id,this.form)
              if(updated.status==200){
                  this.form.unit_kerja = '';
                  this.hideModal();
                  this.$swal({
                    icon: 'success',
                    title: 'Unit Kerja Updated successfully'
                  });
                  this.loadData();
              }
          } catch (e) {
             this.$swal({
                icon: 'Error',
                title: 'Unit Kerja Updated Failed '+e.response.data.errors
              });
      ...
      deleteUnit(id) { 
        this.$swal({
          ...
          if (result.isConfirmed) {
            axios.delete("api/unit/"+id).then(response => {
              this.$swal(
                  'Deleted!',
